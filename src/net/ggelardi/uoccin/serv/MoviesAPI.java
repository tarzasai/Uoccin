...
public class MoviesAPI {
	...
	private static OMDB OMDB_CLIENT;
	
	public static OMDB client() {
		if (OMDB_CLIENT == null) {
			GsonBuilder gb = new GsonBuilder().registerTypeAdapter(List.class, new CommaStringsAdapter());
			//
			OMDB_CLIENT = new RestAdapter.Builder().setEndpoint(API_ENDPOINT).
				...
				}).setLogLevel(RestAdapter.LogLevel.NONE).setClient(new WaitingUCC()).build().create(OMDB.class);
		}
		return OMDB_CLIENT;
	}
